            ...
            loop {
                tokio::select! {
                    ...
                    _ = async { if let Some(ref mut i)=keepalive_interval { i.tick().await; } else { std::future::pending::<()>().await } } => {
                        if let Err(_e) = crate::plugin::events::publish_plugin_keepalive_event(&plugin_name, "system", &format!("Processed {} messages", message_count)).await { consecutive_keepalive_failures +=1; if consecutive_keepalive_failures>=MAX_FAIL { keepalive_interval=None; } } else { consecutive_keepalive_failures=0; }
                    }
                                ...
                                match &typed_msg.content {
                                    ...
                                    ScanMessage::ScanCompleted {
                                        ...
                                        if active_scanners.is_empty() || active_scanners.iter().all(|id| completed_scanners.contains(id)) {
                                            let _ = crate::plugin::events::publish_plugin_completion_event(&plugin_name, "system", "All scanners completed - plugin processing finished").await;
                                            break;
                                        }
                                    },
