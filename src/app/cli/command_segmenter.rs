...
mod tests {
    ...
    fn test_empty_args() {
        ...
        let result = segmenter.segment_arguments(&["repostats".to_string()]).unwrap();
    ...
    fn test_global_args_only() {
        let segmenter = CommandSegmenter::with_commands(vec!["test".to_string()]);
        let args = vec!["repostats".to_string(), "--verbose".to_string(), "--config".to_string(), "file.toml".to_string()];
        
        let result = segmenter.segment_arguments(&args).unwrap();
        
        assert_eq!(result.global_args, vec!["repostats", "--verbose", "--config", "file.toml"]);
    ...
    fn test_multiple_commands() {
        ...
        assert_eq!(result.command_segments[2].args, vec!["--output", "report.html"]);
    }
    ...
    fn test_no_known_commands() {
        ...
        assert_eq!(result.global_args, vec!["repostats", "--verbose", "unknown", "--flag"]);
        assert!(result.command_segments.is_empty());
    }
    ...
    fn test_command_with_equals_syntax() {
        ...
        assert_eq!(result.command_segments.len(), 1);
        assert_eq!(result.command_segments[0].command_name, "scan");
        assert_eq!(result.command_segments[0].args, vec!["--since=1week", "--format=json"]);
    }
