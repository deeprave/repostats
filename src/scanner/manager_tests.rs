...
mod tests {
    ...
    fn test_secure_logging_redaction() {
        ...
        assert_eq!(redacted, "C:\\REDACTED");

        // Test edge case - simple name (no separators)
        ...
        assert_eq!(redacted, "ssh://github.com/REDACTED");
    ...
    #[test]
    fn test_enhanced_secure_logging_redaction_edge_cases() {
        let manager = ScannerManager::new();

        // Git protocol tests
        let git_url = "git://github.com/user/repo.git";
        let redacted = manager.redact_repo_path(git_url);
        assert_eq!(redacted, "git://github.com/REDACTED");

        // File protocol tests
        let file_url = "file:///home/user/repo";
        let redacted = manager.redact_repo_path(file_url);
        assert_eq!(redacted, "file://REDACTED");

        // SSH URL tests with different formats
        let ssh_custom_url = "deploy@server.com:project/repo.git";
        let redacted = manager.redact_repo_path(ssh_custom_url);
        assert_eq!(redacted, "ssh://server.com/REDACTED");

        // Windows UNC path tests
        let unc_full = r"\\server\share\path\to\repo";
        let redacted = manager.redact_repo_path(unc_full);
        assert_eq!(redacted, r"\\server\share\REDACTED");

        let unc_share_only = r"\\server\share";
        let redacted = manager.redact_repo_path(unc_share_only);
        assert_eq!(redacted, r"\\server\share\REDACTED");

        let unc_server_only = r"\\server";
        let redacted = manager.redact_repo_path(unc_server_only);
        assert_eq!(redacted, r"\\server\REDACTED");

        let unc_minimal = r"\\";
        let redacted = manager.redact_repo_path(unc_minimal);
        assert_eq!(redacted, r"\\REDACTED");

        // Windows drive path with forward slashes
        let windows_forward_slash = "C:/Users/user/Documents/repo";
        let redacted = manager.redact_repo_path(windows_forward_slash);
        assert_eq!(redacted, "C:/REDACTED");

        // Relative path tests
        let relative_unix = "user/project/repo";
        let redacted = manager.redact_repo_path(relative_unix);
        assert_eq!(redacted, "REDACTED/repo");

        let relative_windows = r"user\project\repo";
        let redacted = manager.redact_repo_path(relative_windows);
        assert_eq!(redacted, r"REDACTED\repo");

        // Edge cases
        let empty_string = "";
        let redacted = manager.redact_repo_path(empty_string);
        assert_eq!(redacted, "REDACTED");

        // Malformed URLs that should fall back to path handling
        let malformed_url = "https://";
        let redacted = manager.redact_repo_path(malformed_url);
        assert_eq!(redacted, "REDACTED");

        let incomplete_ssh = "git@";
        let redacted = manager.redact_repo_path(incomplete_ssh);
        assert_eq!(redacted, "REDACTED");

        // Complex SSH patterns
        let ssh_port = "git@github.com:22:user/repo.git";
        let redacted = manager.redact_repo_path(ssh_port);
        assert_eq!(redacted, "ssh://github.com/REDACTED");

        // URLs with unusual schemes
        let custom_scheme = "custom://host.example/path/to/repo";
        let redacted = manager.redact_repo_path(custom_scheme);
        assert_eq!(redacted, "custom://REDACTED");

        // Mixed path separators
        let mixed_separators = r"C:\Users/user\Documents/repo";
        let redacted = manager.redact_repo_path(mixed_separators);
        assert_eq!(redacted, r"C:\REDACTED");
    }
}
